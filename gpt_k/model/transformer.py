...
class Transformer(nn.Module):
    def __init__(
        ...
        self.positional_encoding = PositionalEncoding(embedding_dim, 0, seq_len)
    ...
    def forward(self, input):
        ...
        positions = self.positional_encoding(tokens)
        x = self.model(positions)
        y = self.output_probs(x.view(batch_len * seq_len, embedding_dim)).view(batch_len, seq_len, self.vocab_len)
