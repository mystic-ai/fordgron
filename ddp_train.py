...
def infer(rank, world_size, params):
    if rank == 0:
            ...
            wandb.init(project="gpt_k", entity="wjeliot", config=vars(params))
    ...
    for i in tqdm.trange(params.num_batches, position=rank):
        ...
        source = raw[:,0:params.seq_len].to(torch.long)
        ...
        data_test = raw[0,:params.test_size]
        if rank != "cpu":
        ...
        target = raw[:,1:params.seq_len + pad_len + 1].to(torch.long) # [batch_len, seq_len]
        if rank != "cpu":
