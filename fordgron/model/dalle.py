...
class DecoderSelfAttention(AttentionBase):
    ...
    def forward(
        ...
        attention_state[
            :, token_index
        ] = attn_state_new  # select 'all images' and then the active token
        batch_count = decoder_state.size(0)
...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # Self Attention
        self_attn_mask: BoolTensor = (
            self.token_indices < token_index + 1
        )  # mask all 'later' tokens
        self_attn_mask = self_attn_mask[None][
            [0] * decoder_state.size(0)
        ]  # [None] is equivalent of unsqueeze. this expands the mask across all (image_count * 2) 'placeholders'
        ...
        )  # decoder_state [image_count * 2, 1, embedding_dim] attention_state [(image_count * 2) * 2, image_token_count, embedding_dim]
...
class DalleBartDecoder(nn.Module):
    ...
    def forward(
        ...
        token_index_batched = token_index[
            [0] * image_count * 2
        ]  # a tensor of length [image_count * 2], every value being the same index from 0 -> IMAGE_TOKEN_COUNT - 1
        prev_tokens = prev_tokens[
            list(range(image_count)) * 2
        ]  # this is a convoluted way to 'repeat' the tensor values within the same container
        prev_tokens.clamp_(
            0, self.image_vocab_len
        )  # ensuring no image tokens are 'out of the vocab' but if trained properly none should need to be clamped
        decoder_state = self.embed_tokens(
            prev_tokens
        )  # [image_count * 2, embedding_dim]
        decoder_state = decoder_state + self.embed_positions(token_index_batched)
        decoder_state = self.layernorm_embedding(decoder_state)
        decoder_state = decoder_state[:, None]  # [image_count * 2, 1, embedding_dim]
        for i, layer in enumerate(self.layers):
            decoder_state, attention_state[i] = layer(
                decoder_state,
                encoder_state,
                attention_state[i],
                attention_mask,
        ...
        logits = self.lm_head(
            decoder_state
        )  # [image_count * 2, 1, image_vocab_len + 1]
        temperature = settings[[0]]
        top_k = settings[[1]].to(torch.long)
        supercondition_factor = 2 ** settings[[2]]
        logits = logits[:, -1, : self.image_vocab_len]  # cut off final 'loose' logit
        logits: FloatTensor = (
            logits[:image_count] * (1 - supercondition_factor)
            + logits[image_count:] * supercondition_factor
        )  # the first 'half' of the logits are multiplied by 1 - supercondition factor, the second 'half' of the logits are multiplied by the supercondition factor
        logits_sorted, _ = logits.sort(descending=True)
        is_kept = (
            logits >= logits_sorted[:, top_k - 1]
        )  # clip off logits not picked up by top_k
        logits = logits - logits_sorted[:, [0]]
        logits = logits / temperature
        logits.exp_()
        logits = logits * is_kept.to(torch.float32)  # [image_count, image_vocab_len]
        image_tokens = torch.multinomial(logits, 1)[:, 0]  # [image_count]
...
class Decoder(nn.Module):
    ...
    def forward(self, z: FloatTensor) -> FloatTensor:
        z = self.conv_in(z)
        z = self.mid(z)
        for i in reversed(range(5)):  # neat, lol
            z = self.up[i](z)
        z = self.norm_out(z)
        z = z * torch.sigmoid(z)
        z = self.conv_out(z)
...
class VQGanDetokenizer(nn.Module):
    def __init__(self):
        super().__init__()
        vocab_len, embedding_dim = (
            2**14,
            2**8,
        )  # fixed for dalle mini and dalle mega
    ...
    def forward(
        self, is_seamless: bool, z: LongTensor
    ) -> FloatTensor:  # z is image_tokens
        z.clamp_(0, self.vocab_len - 1)
        ...
        if is_seamless:
            ...
            z = self.embedding(z)  # [image_count, image_token_count, image_token_count]
            z = z.view(
                (z.size(0), 2**4, 2**4, 2**8)
            )  # [image_count, ?, ?, image_token_count]

        z = z.permute(0, 3, 1, 2).contiguous()  # [image_count, image_token_count, ?, ?]
        z = self.post_quant_conv(z)
        z = self.decoder(z)
        z = z.permute(
            0, 2, 3, 1
        )  # [image_count, image_token_count, image_token_count, 3]
        z = z.clip(0.0, 1.0) * 255  # clip so all are in the RGB range

        if is_seamless:
            z = z[0]
        else:
            z = z.view(
                [grid_size, grid_size, 2**8, 2**8, 3]
            )  # [image_count, image_token_count, image_token_count, 3]
            z = (
                z.flatten(1, 2).transpose(1, 0).flatten(1, 2)
            )  # [grid_size * image_token_count, grid_size * image_token_count, 3]
...
class DALLE(nn.Module):
    ...
    def image_grid_from_tokens(
        self, image_tokens: LongTensor, is_seamless: bool
    ) -> FloatTensor:
        torch.cuda.empty_cache()
        images = self.detokenizer(
            is_seamless, image_tokens
        )  # [grid_size * 256, grid_size * 256, 3]
        return images
    ...
    def generate_raw_image_stream(
        ...
        with torch.cuda.amp.autocast(dtype=self.dtype):
            ...
            text_tokens = text_tokens[
                expanded_indices
            ]  # [image_count * 2, max_input_text_tokens] duplicate text_tokens out to (2 * image_count) vectors (9 of array1 and 9 of array2)
            encoder_state = encoder_state[
                expanded_indices
            ]  # [image_count * 2, max_input_text_tokens, embedding_dim] duplicate encoder_state out to (2 * image_count) vectors (9 of array1 and 9 of array2)
        ...
        for i in range(IMAGE_TOKEN_COUNT):
            torch.cuda.empty_cache()
            with torch.cuda.amp.autocast(dtype=self.dtype):
                image_tokens[i + 1], attention_state = self.decoder(
                    settings=settings,
                    attention_mask=attention_mask,
                    encoder_state=encoder_state,
                    attention_state=attention_state,
                ...
                )  # image_tokens gets filled out (although I'm not sure why the shape is (token_count, image_count)). the first values remain filled with self.image_vocab_len (I think this is a hack for making the indices nicer (not -1))

            with torch.cuda.amp.autocast(dtype=torch.float32):
                if (
                    (i + 1) % 32 == 0 and progressive_outputs
                ) or i + 1 == 256:  # only yield if all image tokens have been computed (progressive_outputs is streaming)
                    yield self.image_grid_from_tokens(
                        image_tokens=image_tokens[
                            1:
                        ].T,  # skip the first values, but select all, and transpose so that shape is (image_count, token_count)
