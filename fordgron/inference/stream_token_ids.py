...
def stream_token_ids(model, inference_kwargs, input_tokens=None, model_seq_len=2048):
    context = torch.tensor(input_tokens, device=0, dtype=torch.long)
    prev = context
    output = context
    ...
    with torch.no_grad():
        for i in range(inference_kwargs["generation_length"]):
            ...
            logits, previous_generation = model(prev, past_key_values=previous_generation, use_cache=inference_kwargs["use_cache"])
